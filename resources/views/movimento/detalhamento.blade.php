...
@section('content')
    ...
    <div class="d-flex flex-wrap justify-content-center align-items-baseline gap-5 pb-5">
        ...
        <div class="col-md-5 border-start-blue">
            ...
            <p><strong>Status: </strong>{{ $movimento->status }}</p>
            @if ($movimento->tipo == 1)
    ...
    @if (!in_array($movimento->status, ['Aprovado', 'Reprovado']))
        <div class="d-flex justify-content-center gap-4 pt-5 pb-5">
            <button type="button" class="btn btn-ligth btn-lg border-dark" data-bs-toggle="modal" data-bs-target="#modal-recusar">Recusar</button>
            <button type="button" class="btn btn-blue btn-lg" data-bs-toggle="modal" data-bs-target="#modal-aceitar">Aceitar</button>
        </div>
    @endif

    @include('layouts.components.modais.modal-aceitar-recusar')
@endsection
